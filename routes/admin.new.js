...
router.get('/admin', requireAdmin, async (req, res) => {
  try {
    ...
    res.render('admin/dashboard', {
      ...
      posts: [],
      consultations: [],
      dbError: 'Could not reach the database. Check MONGODB_URI in your .env file.',
    });
  }
});
...
router.get('/admin/posts/new', requireAdmin, (req, res) => {
  res.render('admin/post-form', { title: 'New Blog Post', post: null, error: null });
});
...
router.post('/admin/posts', requireAdmin, upload.single('coverImageFile'), async (req, res) => {
  try {
    ...
    res.render('admin/post-form', { title: 'New Blog Post', post: req.body, error: err.message });
  }
});
...
router.get('/admin/posts/:id/edit', requireAdmin, async (req, res) => {
  try {
    ...
    res.render('admin/post-form', { title: 'Edit Blog Post', post, error: null });
  } catch (err) {
    res.redirect('/admin');
  }
});
...
router.put('/admin/posts/:id', requireAdmin, upload.single('coverImageFile'), async (req, res) => {
  try {
    ...
    try {
      ...
      // ignore, fall through with post = null
    }
    res.render('admin/post-form', { title: 'Edit Blog Post', post, error: err.message });
  }
});
...
router.delete('/admin/posts/:id', requireAdmin, async (req, res) => {
  try {
    ...
    // swallow - just bounce back to the dashboard either way
  }
  res.redirect('/admin');
});
